...
def them_sinhvien(connection, metadata, engine,
    ...
    query = db.insert(sinhvien).values(names=names)
    ResultProxy = connection.execute(query)
    # Trả về giá trị id vừa được sinh
    return ResultProxy.inserted_primary_key

#Hàm update
def update_sv(connection,metadata, engine,names,idsv):
    sinhvien = db.Table('sinhvien', metadata, autoload=True, autoload_with=engine)
    query_insert = db.update(sinhvien).values(names=names).where(sinhvien.columns.idsv == idsv)
    ResultProxy = connection.execute(query_insert)
    return ResultProxy

#Hàm delete
def delete_sv(connection,metadata, engine,idsv):
    sinhvien = db.Table('sinhvien', metadata, autoload=True, autoload_with=engine)
